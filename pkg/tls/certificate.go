...
func (c Certificates) GetCertificates() []tls.Certificate {
	...
	for _, certificate := range c {
		...
		if err != nil {
			log.Log.Debug(err, "Error while getting certificate")
			continue
		}
...
func (c *Certificate) AppendCertificate(certs map[string]map[string]*tls.Certificate, storeName string) error {
	...
	if certExists {
		log.Log.Debugf("Skipping addition of certificate for domain(s) %q, to TLS Store %s, as it already exists for this store.", certKey, storeName)
	} else {
		log.Log.Debugf("Adding certificate for domain(s) %s", certKey)
