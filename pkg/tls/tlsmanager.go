...
import (
	...
	"go-faster-gateway/pkg/tls/generate"
...
func (m *Manager) UpdateConfigs(ctx context.Context, stores map[string]Store, configs map[string]Options, certs []*CertAndStores) {
	...
	for optionName, option := range m.configs {
		...
		if option.PreferServerCipherSuites != nil {
			...
			slog.Warnf("TLSOption %q uses `PreferServerCipherSuites` option, but this option is deprecated and ineffective, please remove this option.", optionName)
		}
	}
	...
	for _, conf := range certs {
		if len(conf.Stores) == 0 {
			slog, _ := logger.FromContext(ctx)
			slog.Debugf(fmt.Sprintf("No store is defined to add the certificate %s, it will be added to the default store",
		...
		for _, store := range conf.Stores {
			...
			if err != nil {
				slog.Error("Unable to append certificate %s to store", conf.Certificate.GetTruncatedCertificateName())
			}
		}
	}
	...
	for storeName, storeConfig := range m.storesConfig {
		...
		slog, _ := logger.FromContext(ctx)
		slog.WithFields(map[string]interface{}{
			log.TLSStoreName: storeName,
		})
		ctxStore := logger.NewContext(ctx, slog)

		certificate, err := getDefaultCertificate(ctxStore, storeConfig, st)
		if err != nil {
			slog.WithError(err).Error("Error while creating certificate store")
		}
...
func (m *Manager) Get(storeName, configName string) (*tls.Config, error) {
	...
	tlsConfig.GetCertificate = func(clientHello *tls.ClientHelloInfo) (*tls.Certificate, error) {
		...
		if slices.Contains(clientHello.SupportedProtos, tlsalpn01.ACMETLS1Protocol) {
			...
			if certificate == nil {
				log.Log.Debugf("TLS: no certificate for TLSALPN challenge: %s", domainToCheck)
		...
		if sniStrict {
			log.Log.Debugf("TLS: strict SNI enabled - No certificate found for domain: %q, closing connection", domainToCheck)
			// Same comment as above, as in the isACMETLS case.
			return nil, nil
		}

		if store == nil {
			log.Log.Errorf("TLS: No certificate store found with this name: %q, closing connection", storeName)

			// Same comment as above, as in the isACMETLS case.
			return nil, nil
		}

		log.Log.Debugf("Serving default certificate for request: %q", domainToCheck)
...
func getDefaultCertificate(ctx context.Context, tlsStore Store, st *CertificateStore) (*tls.Certificate, error) {
	...
	slog, _ := logger.FromContext(ctx)
	slog.Debug("No default certificate, fallback to the internal generated certificate")
