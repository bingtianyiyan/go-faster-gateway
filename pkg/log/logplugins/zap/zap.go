...
func (l *zaplog) Init(opts ...logger.Option) error {
	...
	defineEncoder := zapcore.NewJSONEncoder(zapConfig.EncoderConfig)
	...
	for _, w := range writer {
		...
		logCore := zapcore.NewCore(
			...
			zapConfig.Level)
		allCore = append(allCore, logCore)
	}

	core := zapcore.NewTee(allCore...)

	log := zap.New(core, zap.AddCaller(), zap.AddCallerSkip(skip), zap.AddStacktrace(zap.DPanicLevel))
